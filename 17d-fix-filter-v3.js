...
/**
 * ═══════════════════════════════════════════════════════════════
 *  MODULE 17d — FILTRE v3 (renforcé)
 * ═══════════════════════════════════════════════════════════════
 *
 * Ajoute 3 règles pour exclure :
 *   1. Titres avec préfixe pays ("Ghana:", "Togo:", "Sénégal:")
 *   2. Mots politiques ("président", "gouvernement", "ministre")
 *   3. Titres trop longs (> 100 caractères = article de blog)
 *
 * FICHIERS MODIFIÉS (1) :
...
async function main() {
  log.banner("MODULE 17d — FILTRE v3");
  ...
  if (OPTIONS.uninstall) {
    log.info("Ce module ne fait que modifier relevance_filter.py.");
    if (!OPTIONS.dryRun) markUninstalled(MODULE_ID);
    return;
  }

  log.section("Modification de relevance_filter.py");
  ...
  if (!OPTIONS.dryRun) {
    markInstalled(MODULE_ID, {
      ...
      note: "Filtre v3 : prefixes pays + mots politiques + titres longs",
    });
  }

  log.banner("MODULE 17d — TERMINE");

  console.log("");
  console.log("  Nouvelles regles (v3) :");
  console.log("  1. Prefixe pays 'XX:' → rejet automatique");
  console.log("     Ex: 'Ghana: Le president...'");
  console.log("");
  console.log("  2. Mots politiques → rejet automatique");
  console.log("     Ex: 'president', 'gouvernement', 'ministre'");
  console.log("");
  console.log("  3. Titre > 100 car. → -3 points");
  console.log("     Ex: 'Production des savoirs sur le continent...'");
  console.log("");
  console.log("  Prochaines etapes :");
  console.log("  1. Verifier la syntaxe :");
  console.log("     cd backend");
  console.log("     python -c 'from app.services.relevance_filter import filter_relevant_jobs; print(\\'OK\\')'");
  console.log("");
